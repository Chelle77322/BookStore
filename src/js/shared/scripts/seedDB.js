...
//Adding some dummy books into the Novels collection
connect(
...
Novel.remove ({}).then(() => Novel.collection.insertMany(novelSeed)).then(data => {
    console.log(data.result.n + "novels have been inserted");
    process.exit(0);
}).catch(error => {
    console.error(error);
    process.exit(1);
